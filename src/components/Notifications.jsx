...
export default function Notifications({ onNavigate }) {
  ...
  const fetchAllUsers = async () => {
    const first = await apiClient.get(
      `/users/dashboard/all-users?page=1&limit=${USERS_LIMIT}`,
    );
    ...
    for (let p = 2; p <= totalPgs; p++) {
      requests.push(
        apiClient.get(
          `/users/dashboard/all-users?page=${p}&limit=${USERS_LIMIT}`,
        ),
      );
    }
  ...
  const handleSendNotification = async () => {
    try {
      ...
      if (response.data.isRequestSuccessful) {
        ...
        showToastMsg(
          "error",
          response.data.errorDetail?.message || "Failed to send notifications",
        );
      }
    } catch (error) {
  ...
  const uploadImage = async (file) => {
    ...
    try {
      ...
      if (
        response.data.isRequestSuccessful &&
        response.data.successResponse?.url
      ) {
    ...
    try {
      ...
      if (userIds.length === 1) {
        ...
        response = await apiClient.post(
          "/notifications/send-bulk-promotional",
          {
            userIds,
            title: promoTitle,
            body: promoBody,
      ...
      showToastMsg(
        "error",
        "An error occurred while sending promotional notification",
      );
    } finally {
      setPromoSendStatus("");
    }
                ...
                {sortedUsers.length === 0 ? (
                  ...
                  sortedUsers.map((user, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <span
                          className={`${getStatusColor(user.status)} text-white px-3 py-1 rounded-full text-sm font-medium`}
                        >
                          {user.status}
                        </span>
                      </td>
          ...
          <div className="flex items-center justify-between mb-6">
            <p className="text-gray-400 text-sm">
              ...
              <span className="text-white font-medium">
                {(usersPage - 1) * USERS_LIMIT + 1}–
                {Math.min(usersPage * USERS_LIMIT, usersTotalCount)}
              </span>{" "}
              of{" "}
              <span className="text-white font-medium">{usersTotalCount}</span>{" "}
              users
            </p>
          ...
          {activeTab === "promotional" && (
            <>
              ...
              <div className="mb-4">
                ...
                />
                <div className="text-gray-400 text-sm mt-1">
                  {promoBody.length}/1000 characters
                </div>
              </div>
                  ...
                  {promoImagePreview ? (
                    ...
                    <div className="flex flex-col items-center py-4">
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        PNG, JPG, JPEG · Max 5MB (Recommended: 1080 x 1320 px
                        (3:4 portrait))
                      </p>
                    </div>
                  )}
              ...
              <div className="mb-6">
                <label className="block text-gray-300 text-sm mb-2">
                  Promotional Link <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={promoLink}
              ...
              <div className="flex justify-end">
                <button
                  ...
                  disabled={
                    !promoTitle || !promoBody || !promoLink || !!promoSendStatus
                  }
